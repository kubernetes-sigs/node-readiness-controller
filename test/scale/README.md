...
| `JUNIT_REPORT` | `false` | If set to `true`, generates a JUnit XML report (`junit_scalability_report.xml`) with metric properties for TestGrid graph view. |
...
*   **`junit_scalability_report.xml`**: *(Optional, when `JUNIT_REPORT=true`)* A JUnit XML report with phase test cases and custom metric properties for TestGrid graph view.
