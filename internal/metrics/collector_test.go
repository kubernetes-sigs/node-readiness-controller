...
func TestReadinessCollector_NodeListErrorSkipsBothMetrics(t *testing.T) {
	stub := &stubLister{
		...
		inventory: map[RuleModeKey]float64{{EnforcementMode: "continuous", DryRun: false}: 1},
		matched:   map[string]float64{"gpu-ready": 1},
	}
	...
	if len(got["node_readiness_rules"]) != 0 {
		t.Fatalf("expected no node_readiness_rules metrics when ListNodes fails, got %v", got["node_readiness_rules"])
	}
	if len(got["node_readiness_rule_matched_nodes"]) != 0 {
		t.Fatalf("expected no node_readiness_rule_matched_nodes metrics when ListNodes fails, got %v", got["node_readiness_rule_matched_nodes"])
	}

	if stub.gotNodesForRuleStates != nil || stub.gotNodesForBlocked != nil || stub.gotNodesForMatched != nil || stub.gotRulesForInventory != nil {
		t.Fatalf("expected Collect to short-circuit before calling any counting method, but ListRuleNodeStates got %v, ListBlockedNodes got %v, ListRuleMatchedNodes got %v, ListRuleInventory got %v",
			stub.gotNodesForRuleStates, stub.gotNodesForBlocked, stub.gotNodesForMatched, stub.gotRulesForInventory)
	}
}
...
func TestReadinessCollector_RuleListErrorSkipsBothMetrics(t *testing.T) {
	...
	if len(got["node_readiness_rules"]) != 0 {
		t.Fatalf("expected no node_readiness_rules metrics when ListRules fails, got %v", got["node_readiness_rules"])
	}
	if len(got["node_readiness_rule_matched_nodes"]) != 0 {
		t.Fatalf("expected no node_readiness_rule_matched_nodes metrics when ListRules fails, got %v", got["node_readiness_rule_matched_nodes"])
	}

	if stub.gotRulesForRuleStates != nil || stub.gotRulesForBlocked != nil || stub.gotRulesForMatched != nil || stub.gotRulesForInventory != nil {
		t.Fatalf("expected Collect to short-circuit before calling any counting method, but ListRuleNodeStates got %v, ListBlockedNodes got %v, ListRuleMatchedNodes got %v, ListRuleInventory got %v",
			stub.gotRulesForRuleStates, stub.gotRulesForBlocked, stub.gotRulesForMatched, stub.gotRulesForInventory)
	}
}
...
func TestReadinessCollector_ConcurrentCollect(t *testing.T) {
	c := NewReadinessCollector(&stubLister{
		...
		inventory: map[RuleModeKey]float64{
			{EnforcementMode: "continuous", DryRun: false}: 1,
		},
