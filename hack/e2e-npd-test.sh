...
KIND_CLUSTER="${KIND_CLUSTER:-nrr-test-npd}"
...
function startup {
    if [ "$USE_EXISTING_CLUSTER" != 'true' ]; then
        ...
        # Create Kind config to apply startup taint to worker node
...
EOF
        ...
        $KIND create cluster --name "$KIND_CLUSTER" --config "$ARTIFACTS/kind-config.yaml" --image nrr/kind-node:"${K8S_VERSION}" --wait 2m
        
        # Load the controller image into the cluster
        # IMG_PREFIX and IMG_TAG should be passed or we default to controller:latest
        IMG_PREFIX=${IMG_PREFIX:-controller}
        IMG_TAG=${IMG_TAG:-latest}
        $KIND load docker-image "${IMG_PREFIX}:${IMG_TAG}" --name "$KIND_CLUSTER" || true
...
go test -tags=e2e_npd ./test/e2e_npd/ -v -ginkgo.v \
    --ginkgo.junit-report="$ARTIFACTS/junit-npd.xml"
